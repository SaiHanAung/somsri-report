...
<template>
    <span v-if="props.pending" class="loading loading-spinner loading-lg text-[#a52241] absolute left-1/2 top-1/2"></span>
    <div class="bg-[#a52241] rounded-box max-w-fit h-full text-white flex flex-col">
        <div class="py-2 sticky top-0 text-center border-b-2">
            <NuxtLink to="/" @click="refresh" class="flex flex-col hover:bg-transparent focus:text-white">
        ...
        <ul class="menu divide-y h-full overflow-y-auto overflow-x-hidden flex flex-nowrap">
            <li class="py-2 " v-for="(page, i) in pages" :key="'page_' + i">
                <!-- <NuxtLink :to="page.page_url" :class="[ -->
        ...
        <dialog id="edit_modal" class="modal text-[#a52241]">
            <div class="modal-box max-w-xl">
                ...
                <div class="overflow-y-auto overflow-x-hidden" style="height: 32rem;">
                    <div :class="['pt-4 flex gap-3 items-center', data.isEditPage != page.title ? '' : 'border-2 rounded-xl pb-4 pl-4 mt-3']"
                        v-for="(page, i) in pages" :key="'page_' + i">
                        <EditPage v-if="data.isEditPage == page.title" :page="page" />
                        ...
                        <button class="btn btn-sm btn-circle btn-ghost"
                            v-if="data.isLoggedIn && data.isEditPage != page.title" @click="data.isEditPage = page.title">
                            <!-- Edit icon -->
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                stroke="currentColor" class="w-5 h-5">
                                <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        <div v-if="data.isEditPage == page.title" class="grid content-center">
                            ...
                            <button class="btn btn-sm btn-circle btn-ghost">
                                <!-- Delete icon -->
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" @click="pages.splice(i, 1)"
